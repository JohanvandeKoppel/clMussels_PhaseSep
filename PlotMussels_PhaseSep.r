...
on=1;off=0;
setwd('/Simulations/OpenCL/clMussels_PhaseSep/clMussels_PhaseSep')

require(fields)

FID = file("Mussels_PhaseSep.dat", "rb")
...
if (Movie==on) { 
   ...
   OutFile="Mussels_PDE.mp4"
  
   print(paste(" building :", OutFile))
  
   CmdLine=sprintf("ffmpeg -y -r 30 -i %s -c:v libx264 -pix_fmt yuv420p -b:v 5000k %s", InFiles, OutFile)
   cmd = system(CmdLine)
