...
////////////////////////////////////////////////////////////////////////////////
// Main program code for clMussels_CH
////////////////////////////////////////////////////////////////////////////////
...
int main()
{
	...
	float* h_M = (float*)malloc(Grid_Size*sizeof(float));
    ...
    srand(50);
	randomInit(h_M, Grid_Width, Grid_Height);
    ...
    printf("\n");
	printf(" * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n");
	printf(" * Mussel Aggregation model                              * \n");
	printf(" * OpenCL implementation : Johan van de Koppel, 2014     * \n");
	printf(" * Following a model by Liu et al, PNAS 2013             * \n");
	printf(" * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n\n");
    ...
    cl_mem d_J1X = clCreateBuffer(context, CL_MEM_READ_WRITE, Grid_Memory, NULL, &err);
    cl_mem d_J1Y = clCreateBuffer(context, CL_MEM_READ_WRITE, Grid_Memory, NULL, &err);
    cl_mem d_J2  = clCreateBuffer(context, CL_MEM_READ_WRITE, Grid_Memory, NULL, &err);
	...
	cl_kernel kernel_1 = clCreateKernel(program, "Mussels_PDE_Kernel_Phase1", &err);
    if (err!=0) { printf(" > Create Kernel 1 Error : %d \n\n", err); exit(1); }

	/* Set OpenCL kernel arguments */
	err  = err+clSetKernelArg(kernel_1, 0, sizeof(cl_mem), (void *)&d_M);
    err |= err+clSetKernelArg(kernel_1, 1, sizeof(cl_mem), (void *)&d_J1X);
	err |= err+clSetKernelArg(kernel_1, 2, sizeof(cl_mem), (void *)&d_J1Y);
	err |= err+clSetKernelArg(kernel_1, 3, sizeof(cl_mem), (void *)&d_J2);
    if (err!=0) { printf(" > Memory binding error: %d \n\n", err); exit(1); }
    
    /*----------Linking of kernel phase 2-------------------------------------*/
    
    // Create AridLands OpenCL kernel
	cl_kernel kernel_2 = clCreateKernel(program, "Mussels_PDE_Kernel_Phase2", &err);
    if (err!=0) { printf(" > Create Kernel 2 Error : %d \n\n", err); exit(1); }
    
	// Set OpenCL kernel arguments
	err  = err+clSetKernelArg(kernel_2, 0, sizeof(cl_mem), (void *)&d_M);
    err |= err+clSetKernelArg(kernel_2, 1, sizeof(cl_mem), (void *)&d_J1X);
	err |= err+clSetKernelArg(kernel_2, 2, sizeof(cl_mem), (void *)&d_J1Y);
	err |= err+clSetKernelArg(kernel_2, 3, sizeof(cl_mem), (void *)&d_J2);
    if (err!=0) { printf(" > Memory binding error: %d \n\n", err); exit(1); }
	...
	while((float)time_elapsed<=(float)EndTime)
    {
        ...
        if((float)time_elapsed>=(float)RecordTimes[Counter])
        {
            ...
            err = clEnqueueReadBuffer(command_queue, d_M, CL_TRUE, 0,
                    Grid_Width*Grid_Height*sizeof(float), h_M, 0, NULL, NULL);
    ...
    const std::string DataPath = PathName + "Mussels_PhaseSep.dat";
    ...
    err = clReleaseMemObject(d_J1X);
    err = clReleaseMemObject(d_J1Y);
    err = clReleaseMemObject(d_J2);
...
void randomInit(float* data, int x_siz, int y_siz)
{
	for(int i=0;i<y_siz;i++)
	{
		...
		{
			data[i*x_siz+j] = (rand() / (float)RAND_MAX)*1.3;
		}
	}
